...
# ==========================================
# 3. CSS MÁGICO (Estética Premium e Containers)
# ==========================================
st.markdown("""
    <style>
    .block-container {
    ...
    hr.linha-divisoria { border: none; height: 2px; background-color: #B08D6A; margin: 30px 0; }
...
with k2: st.markdown(f'<div class="cartao-kpi"><span class="kpi-titulo">Inflação (Último Mês)</span><span class="kpi-valor">{str_inflacao}</span></div>', unsafe_allow_html=True)
with k3: st.markdown(f'<div class="cartao-kpi"><span class="kpi-titulo">Dólar (Último Mês)</span><span class="kpi-valor">{str_dolar}</span></div>', unsafe_allow_html=True)

st.markdown("<hr class='linha-divisoria'>", unsafe_allow_html=True)

# ==========================================
# 7. A GRANDE DIVISÃO DOS GRÁFICOS (Grids Alinhados)
# ==========================================
        ...
        with c2:
            with st.container(height=520, border=True):
                ...
                if not df_p2.empty:
                    ...
                    labels_completos = [f"{d.strftime('%m/%y')}\n{v}\n{s}" for d, v, s in zip(datas_sel, valores_dolar, valores_selic)]
        ...
        with c3:
            with st.container(height=520, border=True):
                ...
                if not df_p3.empty:
                    ...
                    ax3_1.set_ylabel('Taxa Câmbio (R$)', color='#2980b9', fontsize=9, fontweight='bold')
                    ax3_1.tick_params(axis='y', labelcolor='#2980b9', labelsize=8)
                    ax3_1.tick_params(axis='x', labelsize=8, rotation=45)
                    
                    for bar in bars:
                        height = bar.get_height()
                        ax3_1.text(bar.get_x() + bar.get_width()/2., height + 0.05, f'{height:.1f}', ha='center', va='bottom', fontsize=7, fontweight='bold', color='#1a5276')
        ...
        with c6:
            with st.container(height=520, border=True):
                ...
                if not df_p6.empty:
                    ...
                    ax6.fill_between(df_p6['Data'], 0, 1.05, where=(df_p6['Pressao_Dupla'] > limite_critico), color='red', alpha=0.15, label='Alerta: Risco Skimpflation', interpolate=True)
                ...
                if taxa_projetada is not None:
                    ...
                    if col_preco and col_produto and col_marca:
                        ...
                        if marca_selecionada != "Escolha uma marca...":
                            ...
                            st.info("👆 Selecione uma marca acima para simular o repasse.")
                    else:
                        st.error("Colunas não identificadas na planilha.")
                else:
                    st.info("Aguardando os cálculos da Inteligência Artificial...")